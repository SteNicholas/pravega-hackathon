...
def train_sample_op(train_path, input_dim, nClasses, batch_size, num_epochs):
    ...
    batch_values = tf.train.batch([values], batch_size=batch_size, capacity=64000, enqueue_many=True, num_threads=16,
...
def train(train_path, model_dir, save_name):
    ...
    try:
        ...
        while True:
            iteration += 1
            train_sample_, train_label_, train_one_hot_label_ = read_sess.run(
                [trn_samples, trn_labels, trn_one_hot_label])
            # print train_label_
            ...
            if iteration % 500 == 0:
                ...
                train_pred_ = model_sess.run(pred_op,
                                             feed_dict={x_ph: train_sample_, one_hot_label_ph: train_one_hot_label_})
                ...
                result = model_sess.run(merged, feed_dict={x_ph: train_sample_,
                                                           one_hot_label_ph: train_one_hot_label_})  # 计算需要写入的日志数据
