...
public class StreamerUtil {
    ...
    public static PravegaConfig appendPravegaProps(FlinkStreamerConfig config) {
        PravegaConfig pravegaConfig =
                PravegaConfig.fromDefaults()
                        .withControllerURI(URI.create(config.pravegaControllerUri))
                        .withDefaultScope(config.pravegaScope)
                        .withHostnameValidation(config.pravegaValidateHostname)
                        .withTrustStore(config.pravegaTrustStore);

        if (config.pravegaAuthType != null
                && config.pravegaAuthToken != null
                && !isCredentialsLoadDynamic()) {
            pravegaConfig.withCredentials(
                    new FlinkPravegaUtils.SimpleCredentials(
                            config.pravegaAuthType, config.pravegaAuthToken));
        }
    ...
    public static Schema getSourceSchema(org.apache.flink.configuration.Configuration conf) {
        if (conf.getOptional(FlinkOptions.READ_AVRO_SCHEMA_PATH).isPresent()) {
            ...
            final String errorMsg =
                    String.format(
                            "Either option '%s' or '%s' "
                                    + "should be specified for avro schema deserialization",
                            FlinkOptions.READ_AVRO_SCHEMA_PATH.key(),
                            FlinkOptions.READ_AVRO_SCHEMA.key());
    ...
    public static DFSPropertiesConfiguration readConfig(
            FileSystem fs, Path cfgPath, List<String> overriddenProps) {
     ...
     *
     * <p>If the class name of key generator is configured through the properties file, i.e., {@code
     ...
     * <p>If the class name of key generator is configured through the properties file, i.e., {@code
     * props}, use the corresponding key generator class; otherwise, use the default key generator
     * class specified in {@link FlinkOptions}.
     */
    public static KeyGenerator createKeyGenerator(Configuration conf) throws IOException {
        ...
        try {
            return (KeyGenerator)
                    ReflectionUtils.loadClass(keyGeneratorClass, flinkConf2TypedProperties(conf));
        } catch (Throwable e) {
            throw new IOException("Could not load key generator class " + keyGeneratorClass, e);
        }
    }
    ...
    public static HoodieRecordPayload createPayload(
            String payloadClass, GenericRecord record, Comparable orderingVal) throws IOException {
        try {
                        ...
                        .withCompactionConfig(
                                HoodieCompactionConfig.newBuilder()
                                        ...
                                        .withMaxNumDeltaCommitsBeforeCompaction(
                                                conf.getInteger(
                                                        FlinkOptions.COMPACTION_DELTA_COMMITS))
                                        .withMaxDeltaSecondsBeforeCompaction(
                                                conf.getInteger(
                                                        FlinkOptions.COMPACTION_DELTA_SECONDS))
                                        .build())
    ...
    /**
     * Converts the give {@link Configuration} to {@link TypedProperties}. The default values are
     * also set up.
     *
    ...
    public static void initTableIfNotExists(Configuration conf) throws IOException {
        ...
        if (!fs.exists(new Path(basePath, HoodieTableMetaClient.METAFOLDER_NAME))) {
            ...
            LOG.info(
                    "Table [{}/{}] already exists, no need to initialize the table",
                    basePath,
